...
const m4mul = (...args) => args.reduce((m1,m2) => {
  if(m2.length == 4) return m2.map((e,i)=>m4rowdot(m1,i,m2))
  if(m1.length == 4) return m1.map((e,i)=>m4rowdot(m2,i,m1))
...
const m3mul = (...args) => args.reduce((m1,m2) => {
  if(m2.length == 3) return m2.map((e,i)=>m3rowdot(m1,i,m2))
  if(m1.length == 3) return m1.map((e,i)=>m3rowdot(m2,i,m1))
